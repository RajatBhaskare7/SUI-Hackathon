...
export default function SignIn() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      axios.get(process.env.REACT_APP_BASE_URL + '/admin/getadmin')
          .then((res) => {
              const admin = res.data.find((user) => user._id === id && user.role === 'admin');
              ...
              if (!admin) {
                ...
                setSuccess(true);
                setMessage('You are Already Login as Admin');
                setTimeout(() => {
                  setSuccess(false);
  ...
  const handlesubmit = (e) => {
    ...
    axios.post(process.env.REACT_APP_BASE_URL+'/auth/admin/login', data).then((res) => {
      if (res.status === 200) {
  ...
  return (
    ...
    <div class="flex p-4">
      ...
      <div class="ms-3">
        ...
        You are Already Login as User Please Logout First
        </p>
      </div>
    </div>
  </div>
    ...
    <div class="w-full ">
      ...
      <form class="mt-6" >
        ...
        <div class="text-right mt-2">
          ...
          <Link to="/auth/admin/forgotpassword" class="text-sm font-semibold text-gray-700 hover:text-blue-700 focus:text-blue-700">Forgot Password?
            </Link>
        </div>
